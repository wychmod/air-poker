...
- Bet 轮次固定为：玩家动作 -> AI 响应；若 AI raise/all-in，玩家再响应一次后结束。
- 下注阶段双方都不知道对方锁定的上层成手。
...
- 牌型比较中 A = 14（最强）、K = 13、Q = 12、J = 11、2 = 2（最弱）。A-2-3-4-5 不判为顺子（V1 不实现低 A 顺子）；A-K-Q-J-10 才是高 A 顺子。具体牌型序位见 `04-hand-evaluator.md`。
...
4. HandEvaluator 10 种牌型、A-2-3-4-5 不算顺子、同牌型比较、有效牌数量降级（5/4/3/2/1/0 张）测试。
