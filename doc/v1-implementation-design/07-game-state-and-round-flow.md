...
| `upperSelect` | `publicTargets`, `playerCandidateHands`, `playerPossibleHandSummary`, `aiLockedHand?` | 玩家锁定成手；AI 锁定成手由系统 action `aiLockedHand` 注入 |
| `betting` | `publicTargets`, `playerLockedHand`, `aiLockedHand`, `betState` | 进入 Bet 后两者均不可改 |
| `showdown` | `publicTargets`, `playerLockedHand`, `aiLockedHand`, `showdown` | 公开双方成手 + 牌型 + 重叠 |
| `resolve` | `publicTargets`, `playerLockedHand`, `aiLockedHand`, `resolution` | 结算 |
| `roundSummary` | `publicTargets`, `playerLockedHand`, `aiLockedHand`, `resolution` | 等待用户继续 |
...
- 成功：`{ ok: true, state }`，进入 `roundStart` 或已经准备进入 `lowerSelect` 的状态。
...
1. 先扣呼吸成本：双方各扣 1 Air。
   - 双方都足够：进入步骤 2。
   - 某方不足 1：写入 `lastError.code = "cannotPayBreathingCost"`，进入 `gameOver`，`roundNumber` 不递增，**不**进入下一回合。
2. 再扣参加费：双方各扣 `R` Air（`R = roundNumber`；决胜回合 `R = 5`）。
   ...
   - 某方不足 R：写入 `lastError.code = "cannotPayAnte"`，进入 `gameOver`。**呼吸扣减不回滚**（按原作口径"先扣后判"），该方 Air 可能已为负时 gameOver 触发 game over。
...
- 若 `roundNumber === 5` 且双方 Air 相同：
  - 比 `roundHistory` 累计 `playerPool` vs `aiPool`（取自 `resolution.escrowDistribution.playerReceivedBet - escrow.playerBet` 之和，或等价于净赢得底池），分出胜负则进入 `gameOver`。
...
- `startNewGame(seed?)`：seed 可选；不传则由 `app-service` 的 `createRuntimeSeed` 生成。
...
- `restartGame()`：从 gameOver / 任意阶段"放弃当前牌局"。**V1 钉死语义**：必须传入新 seed（默认调 `createRuntimeSeed`）；不复用当前 seed。`restartGame` 写入 `lastError` 提示用户"已放弃当前牌局，开始新局"。
- `openPanel(panelType)`：打开只读面板（`rulesOpen / historyOpen / discardPileOpen / settingsOpen`）。
- `closePanel(panelType?)`：关闭只读面板（不传则关闭全部）。
...
- 无法支付呼吸成本立即结束（`cannotPayBreathingCost`），`roundNumber` 不递增。
- 无法支付参加费立即结束（`cannotPayAnte`），**呼吸扣减不回滚**。
