...
最低测试清单：
- 52 张牌构建后无重复 ID。
- A、J、Q、K 点数映射正确，其中 A 求和时为 1。
- 数字牌生成满足 `玩家数字牌总和 + AI 数字牌总和 + burnCards 点数和 = 364`。
- 玩家与 AI 数字牌总和差距不超过设计阈值。
- `HandSolver` 只返回 5 张牌。
- `HandSolver` 返回组合点数和必须等于目标值。
- `HandSolver` 允许返回包含弃牌区用过牌的组合，且每张牌正确标记「未用过 / 已用过」状态。
- `HandEvaluator` 覆盖 10 种牌型。
- `HandEvaluator` 正确处理原作牌型序位。
- 同牌型比较有确定结果。
- Bet 动作合法性正确。
- Raise 上限为当前场上 Bet 总额的 `1/2`。
- 本回合总 Bet 不超过双方剩余可下注 Air 较小值。
- Fold 后胜负结算正确。
- 灾厄只惩罚输家。
- 灾厄额外 Air 从系统中消失，不归属任何一方。
- 状态机主路径可从新局跑到游戏结束。
- AI 决策不能读取玩家隐藏成手字段。
