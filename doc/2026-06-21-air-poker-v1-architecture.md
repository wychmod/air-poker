...
- 执行 V1 多轮下注收敛规则：
  - 玩家先动，双方轮流行动。
  - 任一方 `check / call / fold` 即 Bet 结束（`check` 仅无压力时合法；`call` 补齐后双方下注相等）。
  - 任一方 `raise` 后轮到对方响应，对方可再 `raise`（受 min-raise）。
  - 任一方 `all-in` 后对方只能 `call / fold`，响应后 Bet 结束。
  - 由 Air 与总 Bet 上限自然终止，不设固定轮数上限。

关键约束：
- Raise 增量 ≥ 上次 raise 增量（德州 min-raise）；增量 ≤ 行动方剩余可下注 Air。
